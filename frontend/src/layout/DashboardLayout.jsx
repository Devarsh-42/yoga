...
const DashboardLayout = () => {
    ...
    return (
        <div className="flex">
            <div
                className={`${open ? "w-72 overflow-y-auto" : "w-[90px] overflow-auto"
                    } bg-teal-50 h-screen p-5 hidden md:block pt-8 relative duration-300`}
            >
                <div className="flex gap-x-4 items-center">
                    <img
                        src='/src/assets/navbar/yoga-logo.png'
                        onClick={() => setOpen(!open)}
                        className={`cursor-pointer h-[40px] duration-500 ${open && "rotate-[360deg]"
                            }`}
                    />
                    <h1
                        className={`text-teal-600 cursor-pointer font-bold origin-left text-xl duration-200 ${!open && "scale-0"
                            }`}
                        onClick={() => setOpen(!open)}
                    >
                        Krushnapriya Yog
                        ...
                        {role === 'admin' && adminNavItems.map((menuItem, index) => (
                            <li key={index} className="mb-2">
                                <NavLink
                                    to={menuItem.to}
                                    className={({ isActive }) =>
                                        `flex ${isActive ? "bg-orange-500 text-white " : "text-gray-700"
                                        }  duration-150 rounded-md p-2 cursor-pointer hover:bg-teal-500 hover:text-white font-bold text-sm items-center gap-x-4  `
                                    }
                                >
                                    {menuItem.icon}
                                    <span className={`${!open && "hidden"} origin-left duration-200`}>
                ...
                <ul className="pt-6">
                    <p className={`ml-3 uppercase text-gray-500 ${!open && "hidden"}`}><small>Useful Links</small></p>
                    {lastMenuItems.map((menuItem, index) => (
                        <li key={index} className="mb-2">
                            <NavLink
                                to={menuItem.to}
                                className={({ isActive }) =>
                                    `flex ${isActive ? "bg-orange-100 text-orange-700" : "text-gray-700"
                                    }  duration-150 rounded-md p-2 cursor-pointer hover:bg-orange-100 hover:text-orange-500 font-bold text-sm items-center gap-x-4  `
                                }
                            >
                                {menuItem.icon}
                                <span className={`${!open && "hidden"} origin-left duration-200`}>
                    ...
                    <li>
                        <button
                            onClick={() => handleLogout()}
                            className="flex text-gray-700 duration-150 rounded-md p-2 cursor-pointer hover:bg-orange-500 hover:text-white font-bold text-sm items-center gap-x-4 w-full"
                        >
                            <BiLogInCircle className='text-2xl' />
                            <span className={`${!open && "hidden"} origin-left duration-200`}>
                                Logout
            ...
            <div className="h-screen overflow-y-auto px-8 flex-1 bg-white">
                <Scroll />
                <Outlet />
                <ToastContainer />
            </div>
